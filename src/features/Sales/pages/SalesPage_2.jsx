...
export default function SalesPage() {
  ...
  const storeName    = settings?.storeName    ?? settings?.store_name    ?? 'المتجر';
  const cashierName  = settings?.cashierName  ?? settings?.cashier_name  ?? 'الكاشير';
  const taxRate      = parseFloat(settings?.taxRate ?? settings?.tax_rate ?? 0) / 100;
  const defaultPayment = settings?.defaultPaymentMethod ?? settings?.payment_method ?? 'CASH';
  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      ...
      // ── Build receipt using live settings values ──────────────────────
      const receipt = {
        ...
        cashierName,                          // ← from settings
        storeName,                            // ← from settings
        paymentMethod : paymentMethod === 'CASH' ? 'نقدي 💵' : 'بطاقة 💳',
        amountPaid    : cartTotal,
        change        : 0,
        items: cart.filter(Boolean).map((item, idx) => ({
          id         : item.id           ?? idx,
          name       : getDisplayName(item),  // ← always Arabic
          emoji      : item.emoji        ?? '📦',
  ...
  }, [cart, cartSubtotal, discountAmount, taxAmount, taxRate, cartTotal,
      isSubmitting, paymentMethod, cashierName, storeName, showToast]);
